...
CModMedFilt::CModMedFilt()
{
	...
	m_LeftLFO.m_fFrequency_Hz = 0;
	m_LeftLFO.m_uOscType = 0;
	m_LeftLFO.m_uPolarity = 1;
	m_RightLFO.m_fFrequency_Hz = 0;
	m_RightLFO.m_uOscType = 0;
	m_RightLFO.m_uPolarity = 1;
	m_MixLFO.m_fFrequency_Hz = 0;
	m_MixLFO.m_uOscType = 2;
	m_MixLFO.m_uPolarity = 1;
...
bool __stdcall CModMedFilt::prepareForPlay()
{
	...
	//LFO
	m_fMixPct = m_fMix/100.0;
	updateLFO();
	return true;
}

void CModMedFilt::updateLFO()
{
	//Left LFO
	m_LeftLFO.setSampleRate(m_nSampleRate);
	m_LeftLFO.m_uOscType = m_uWaveformLeft;
	m_LeftLFO.m_uPolarity = 1;
	m_LeftLFO.m_fFrequency_Hz = m_fLFORateLeft;
	m_LeftLFO.cookFrequency();
	//Right LFO
	m_RightLFO.setSampleRate(m_nSampleRate);
	m_RightLFO.m_uOscType = m_uWaveformRight;
	m_RightLFO.m_uPolarity = 1;
	m_RightLFO.m_fFrequency_Hz = m_fLFORateRight;
	m_RightLFO.cookFrequency();
	//Mix LFO
	m_MixLFO.setSampleRate(m_nSampleRate);
	m_MixLFO.m_uOscType = 2;
	m_MixLFO.m_uPolarity = 1;
	m_MixLFO.m_fFrequency_Hz = m_fMixModRate;
	m_MixLFO.cookFrequency();
}
...
bool __stdcall CModMedFilt::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	float xnL = pInputBuffer[0];
	float xnR = pInputBuffer[1];
	float ynL = 0.0;
	float ynR = 0.0;
	float fMixPctQ = m_fMixPct;

	if(m_uMixMod)
		m_MixLFO.doOscillate(&m_fMixPct, &fMixPctQ);

	if(m_uDoModulateLeft)
	{
		float fModVal = pInputBuffer[0];
		float fModValQ = 0.0;
		if(m_uModSourceLeft == LFO)
			m_LeftLFO.doOscillate(&fModVal, &fModValQ);
		m_LeftFilt.processAudio(xnL, ynL, fModVal);
	}
	else
		m_LeftFilt.processAudio(xnL, ynL);

	pOutputBuffer[0] = m_fMixPct*ynL + (1.0 - m_fMixPct)*pInputBuffer[0];
	...
	if(uNumInputChannels == 2 && uNumOutputChannels == 2)
	{
		if(m_uDoModulateLeft)
		{
			float fModVal = pInputBuffer[1];
			float fModValQ = 0.0;
			if(m_uModSourceLeft == LFO)
				m_RightLFO.doOscillate(&fModVal, &fModValQ);
			m_RightFilt.processAudio(xnR, ynR, fModVal);
		}
		else
			m_RightFilt.processAudio(xnR, ynR);

		pOutputBuffer[1] = m_fMixPct*ynR + (1.0 - m_fMixPct)*pInputBuffer[1];
	}
...
-----------------------------------------------
	...
	m_uWaveformRight                  11
	m_fLFORateRight                   12
	m_uModSourceRight                 13
	m_uDoModulateLeft                 45
	m_uStereoLink                     46
	m_uMixMod                         47
	m_uDoModulateRight                48
...
bool __stdcall CModMedFilt::userInterfaceChange(int nControlIndex)
{
	...
	switch(nControlIndex)
	{
		...
		case 3:
		{
			if(m_uModSourceLeft == SELF)
			{
				m_LeftFilt.setPolarity(true);
				if(m_uStereoLink)
					m_RightFilt.setPolarity(true);
			}
			else
			{
				m_LeftFilt.setPolarity(false);
				if(m_uStereoLink)
					m_RightFilt.setPolarity(false);
			}
			break;
		}
		case 4:
		...
		case 11:
		{
			m_RightLFO.m_uOscType = m_uWaveformRight;
			if(m_uStereoLink)
				m_LeftLFO.m_uOscType = m_uWaveformRight;
			break;
		}
		case 12:
		{
			m_RightLFO.m_fFrequency_Hz = m_fLFORateRight;
			if(m_uStereoLink)
				m_LeftLFO.m_fFrequency_Hz = m_fLFORateRight;
			break;
		}
		case 13:
		{
		...
		case 48:
		{
			...
			if(m_uStereoLink)
			{
				m_LeftFilt.setExtModulation(!m_RightFilt.getExtModulation());
				m_uDoModulateLeft = m_uDoModulateRight;
			}
			break;
		}
		default:
...
// **--0xDEA7--**
	...
	ui1->m_pUserCookedUINTData = &m_uWaveformLeft;
	ui1->cControlUnits = "                                                                ";
	ui1->cVariableName = "m_uWaveformLeft";
	...
	ui2->m_pUserCookedFloatData = &m_fLFORateLeft;
	ui2->m_pUserCookedDoubleData = NULL;
	ui2->m_pUserCookedUINTData = NULL;
	ui2->cControlUnits = "                                                                ";
	ui2->cVariableName = "m_fLFORateLeft";
	...
	ui3->m_pUserCookedUINTData = &m_uModSourceLeft;
	ui3->cControlUnits = "                                                                ";
	ui3->cVariableName = "m_uModSourceLeft";
	ui3->cEnumeratedList = "LFO,SELF";
	...
	ui4->fInitUserFloatValue = 50.000000;
	ui4->fInitUserDoubleValue = 0;
	ui4->fInitUserUINTValue = 0;
	ui4->m_pUserCookedIntData = NULL;
	ui4->m_pUserCookedFloatData = &m_fMix;
	ui4->m_pUserCookedDoubleData = NULL;
	ui4->m_pUserCookedUINTData = NULL;
	ui4->cControlUnits = "                                                                ";
	ui4->cVariableName = "m_fMix";
	ui4->cEnumeratedList = "SEL1,SEL2,SEL3";
	ui4->dPresetData[0] = 50.000000;ui4->dPresetData[1] = 0.000000;ui4->dPresetData[2] = 0.000000;ui4->dPresetData[3] = 0.000000;ui4->dPresetData[4] = 0.000000;ui4->dPresetData[5] = 0.000000;ui4->dPresetData[6] = 0.000000;ui4->dPresetData[7] = 0.000000;ui4->dPresetData[8] = 0.000000;ui4->dPresetData[9] = 0.000000;ui4->dPresetData[10] = 0.000000;ui4->dPresetData[11] = 0.000000;ui4->dPresetData[12] = 0.000000;ui4->dPresetData[13] = 0.000000;ui4->dPresetData[14] = 0.000000;ui4->dPresetData[15] = 0.000000;
	ui4->cControlName = "Mix";
	...
	ui5->m_pUserCookedFloatData = &m_fMixModRate;
	ui5->m_pUserCookedDoubleData = NULL;
	ui5->m_pUserCookedUINTData = NULL;
	ui5->cControlUnits = "                                                                ";
	ui5->cVariableName = "m_fMixModRate";
