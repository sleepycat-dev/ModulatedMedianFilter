...
{
	//Filters
	m_LeftFilt.setBufferSize(m_nBufferSize);
	m_RightFilt.setBufferSize(m_nBufferSize);
...
bool __stdcall CModMedFilt::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	if(m_uDoModulate)
	{
		...
		float fModValQ = 0.0;
		if(m_uModSource == LFO)
			m_LFO.doOscillate(&fModVal, &fModValQ);
		m_LeftFilt.processAudio(pInputBuffer[0], pOutputBuffer[0], fModVal);
	}
	else
		m_LeftFilt.processAudio(pInputBuffer[0], pOutputBuffer[0]);
	...
	if(uNumInputChannels == 2 && uNumOutputChannels == 2)
	{
		if(m_uDoModulate)
		{
			...
			if(m_uModSource == LFO)
				m_LFO.doOscillate(&fModVal, &fModValQ);
			m_LeftFilt.processAudio(pInputBuffer[1], pOutputBuffer[1], fModVal);
		}
		else
			m_LeftFilt.processAudio(pInputBuffer[1], pOutputBuffer[1]);
	}
...
bool __stdcall CModMedFilt::userInterfaceChange(int nControlIndex)
{
	...
	switch(nControlIndex)
	{
		...
		case 45:
		{
			m_LeftFilt.setExtModulation(true);
			m_RightFilt.setExtModulation(true);
			break;
		}
		default:
			break;
...
// **--0xDEA7--**
	...
	ui1->m_pUserCookedUINTData = &m_uWaveform;
	ui1->cControlUnits = "                                                                ";
	ui1->cVariableName = "m_uWaveform";
	...
	ui2->m_pUserCookedFloatData = &m_fLFORate;
	ui2->m_pUserCookedDoubleData = NULL;
	ui2->m_pUserCookedUINTData = NULL;
	ui2->cControlUnits = "                                                                ";
	ui2->cVariableName = "m_fLFORate";
	...
	ui3->m_pUserCookedUINTData = &m_uModSource;
	ui3->cControlUnits = "                                                                ";
	ui3->cVariableName = "m_uModSource";
	...
	ui4->m_pUserCookedUINTData = &m_uDoModulate;
	ui4->cControlUnits = "";
	ui4->cVariableName = "m_uDoModulate";
